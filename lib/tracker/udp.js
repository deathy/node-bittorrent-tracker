	...
	server.on("message", function (msg, rinfo) {
		...
		switch (action) {
			...
			case 1:
				...
				var torrent = trackerInstance.getTorrent(infoHash);
				...
				peer = torrent.addPeer(peerId, peer, event);
				...
				w.writeInt(torrent.leechers, 4); // Leechers
				w.writeInt(torrent.seeders, 4); // Seeders

				var len = torrent.writePeers(w.getBuffer(), want, peer);
				w.resize(20 + len);
				break;
			...
			case 2:
				...
				for (var i = 0; i < count; i++) {
					...
					var torrent = trackerInstance.getTorrent(infoHash);
					w.writeInt(torrent.seeders, 4); // Seeders
					w.writeInt(torrent.downloads, 4); // Completed
					w.writeInt(torrent.leechers, 4); // Leechers
				}
				break;
		}
