...
function createServer(trackerInstance, port, host) {
	var server = http.createServer(function (request, response) {
        ...
        try {
            ...
            var torrent = trackerInstance.getTorrent(query["info_hash"]);
            var peer = tracker.Peer(remoteIp, query["port"], query["left"]);
            peer = torrent.addPeer(query["peer_id"], peer, tracker.event(query["event"]));
            ...
            var len = torrent.writePeers(peerBuffer, want, peer);
            peerBuffer = peerBuffer.slice(0, len);

            var resp = "d8:intervali"+ tracker.ANNOUNCE_INTERVAL +"e8:completei"+ torrent.seeders +"e10:incompletei"+ torrent.leechers +"e10:downloadedi"+ torrent.downloads +"e5:peers"+ len +":";
